...
pub async fn get_live_logs(state: &AppState) -> Result<String> {
    ...
    // Get process info
    let process_output = machine.exec("pgrep -a orion || echo 'Orion process not found'").await?;
...
pub async fn get_scorpio_status(state: &AppState) -> Result<serde_json::Value> {
    ...
    for (name, path) in paths {
        ...
        let exists_output = machine.exec(&format!("test -e {} && echo 'exists' || echo 'not_found'", path)).await?;
        let exists = String::from_utf8_lossy(&exists_output.stdout).contains("exists");

        // Check if it's a mount point
        let mount_output = machine.exec(&format!("mountpoint -q {} && echo 'mounted' || echo 'not_mounted'", path)).await?;
        ...
        let file_count = if exists {
            let count_output = machine.exec(&format!("find {} -maxdepth 1 -type f 2>/dev/null | wc -l", path)).await?;
            String::from_utf8_lossy(&count_output.stdout).trim().to_string()
        } else {
            "N/A".to_string()
        };
        ...
        let dir_count = if exists {
            let count_output = machine.exec(&format!("find {} -maxdepth 1 -type d 2>/dev/null | wc -l", path)).await?;
            String::from_utf8_lossy(&count_output.stdout).trim().to_string()
        } else {
            "N/A".to_string()
        };
        ...
        let contents: String = if exists {
            let ls_output = machine.exec(&format!("ls -la {} 2>/dev/null | head -20", path)).await?;
    ...
    let orion_output = machine.exec("pgrep -a orion || echo 'Orion not running'").await?;
    let orion_info = String::from_utf8_lossy(&orion_output.stdout);

    // Get Scorpio process (if running)
    let scorpio_output = machine.exec("pgrep -a scorpio || echo 'Scorpio not running'").await?;
