...
const ORION_SOURCE_DIR: &str = "/home/ubuntu/mega/orion";
const ORION_TARGET_DIR: &str = "/home/orion/orion-runner";

/// Upload a single file to the VM via SFTP
async fn upload_file(machine: &KeepAliveMachine, local: impl AsRef<std::path::Path>, remote: impl AsRef<std::path::Path>) -> Result<()> {
    let local_path = local.as_ref();
    let remote_path_str = remote.as_ref().to_string_lossy().into_owned();
    info!("[orion-deploy] Uploading file: {} -> {}", local_path.display(), remote_path_str);
    machine.upload(local, remote).await?;
    Ok(())
}

/// Deploy Orion files to the VM
pub async fn deploy_orion_in_vm(machine: &KeepAliveMachine) -> Result<()> {
    info!("[orion-deploy] Starting Orion deployment");

    // Step 1: Create target directory
    info!("[orion-deploy] Creating directory: {}", ORION_TARGET_DIR);
    machine.exec(&format!("mkdir -p {}", ORION_TARGET_DIR)).await?;
    ...
    for file in &files {
        ...
        if local_path.exists() {
            ...
            upload_file(machine, &local_path, format!("{}/{}", ORION_TARGET_DIR, file)).await?;
        } else {
            info!("[orion-deploy] Skipping missing file: {}", local_path.display());
        }
    }
    ...
    let service_source = PathBuf::from(ORION_SOURCE_DIR).join("systemd").join("orion-runner.service");
    if service_source.exists() {
        info!("[orion-deploy] Uploading systemd service file");
        upload_file(machine, &service_source, "/etc/systemd/system/orion-runner.service").await?;
    } else {
        info!("[orion-deploy] Skipping missing service file");
    }

    // Step 5: Upload orion binary (large file, ~500MB)
    let orion_binary = PathBuf::from("/home/ubuntu/mega/target/debug/orion");
    if orion_binary.exists() {
        let size = std::fs::metadata(&orion_binary).map(|m| m.len()).unwrap_or(0) / 1024 / 1024;
        info!("[orion-deploy] Uploading Orion binary ({} MB)...", size);
        upload_file(machine, &orion_binary, format!("{}/orion", ORION_TARGET_DIR)).await?;
        info!("[orion-deploy] Orion binary uploaded successfully");
    } else {
        return Err(anyhow::anyhow!("Orion binary not found at {:?}", orion_binary));
    }

    // Step 6: Set permissions
    info!("[orion-deploy] Setting executable permissions");
    machine.exec(&format!("chmod +x {}/run.sh", ORION_TARGET_DIR)).await?;
    machine.exec(&format!("chmod +x {}/preflight.sh", ORION_TARGET_DIR)).await?;
    machine.exec(&format!("chmod +x {}/cleanup.sh", ORION_TARGET_DIR)).await?;
    machine.exec(&format!("chmod +x {}/orion", ORION_TARGET_DIR)).await?;
    info!("[orion-deploy] Setting capabilities on orion binary");
    machine.exec("setcap cap_dac_read_search+ep /home/orion/orion-runner/orion").await?;
...
pub async fn start_orion_in_vm(machine: &KeepAliveMachine) -> Result<String> {
    ...
    machine.exec("mkdir -p /data/scorpio/antares/{upper,cl,mnt}").await?;
    ...
    machine.exec("chown -R orion:orion /workspace/mount").await?;
    ...
    info!("[orion-deploy] Orion service start command executed, exit code: {}", start_result.status);
    ...
    if status == "active" {
        ...
        let logs_result = machine.exec("journalctl -u orion-runner --no-pager -n 50 2>&1 || echo 'journalctl failed'").await;
    ...
    let process_result = machine.exec("pgrep -a orion || echo 'Orion process not found'").await?;
    let process_info = String::from_utf8_lossy(&process_result.stdout).trim().to_string();
...
pub async fn replace_env_vars_in_vm(machine: &KeepAliveMachine, target_config: &TargetConfig, target_name: &str) -> Result<()> {
    ...
    info!("[env] Replacing environment variables for target: {}", target_name);
    ...
    // Verify replacements
    let verify_env = machine.exec("grep SERVER_WS /home/orion/orion-runner/.env").await?;
    info!("[env] .env SERVER_WS: {}", String::from_utf8_lossy(&verify_env.stdout).trim());

    let verify_base_url = machine.exec("grep base_url /home/orion/orion-runner/scorpio.toml").await?;
    info!("[env] scorpio.toml base_url: {}", String::from_utf8_lossy(&verify_base_url.stdout).trim());

    info!("[env] Environment variable replacement completed for target: {}", target_name);
    Ok(())
}
