...
将硬编码常量 `ORION_SOURCE_DIR` 和 `/home/ubuntu/mega/target/debug/orion` 全部去掉，`deploy_orion_in_vm` 函数签名增加 `artifact: &ResolvedArtifact` 参数。内部所有路径替换为：
...
- `PathBuf::from("/home/ubuntu/mega/target/debug/orion")` → `artifact.orion_binary.clone()`
...
`orion_artifact` 字段缺失或 `manifest_url` 为空 → 回退到旧行为，从 `/home/ubuntu/mega/...` 本地读，保留开发者本地迭代体验。`artifact_fetcher::ensure_latest` 在该情况下返回一个指向本地路径的 `ResolvedArtifact`，下游代码无差别。
...
- [ ] [`src/orion_deployer.rs`](src/orion_deployer.rs) 去掉硬编码常量 `ORION_SOURCE_DIR` 和 `/home/ubuntu/mega/target/debug/orion`，`deploy_orion_in_vm` 接受 `ResolvedArtifact` 参数
...
- [ ] `manifest_url` 缺失时 fallback 到本地 `/home/ubuntu/mega` 路径，保留本地开发体验
